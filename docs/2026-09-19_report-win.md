...
| `printer-dialog-probe.rkt` (§43.7, echter interaktiver Dialog-Pfad) | **Weiterhin nicht abschließend testbar — Befund bleibt offen, nicht als Automatisierungsgrenze bestätigt.** 2 von 2 Versuchen: `get-page-setup-from-user` blockiert nach `can-get-page-setup-from-user? = #t`, kein Sekundärfenster erscheint innerhalb von 15s (UIAutomation-Enumeration über `ProcessIdProperty` liefert nichts, Prozess muss danach hart beendet werden). **Neuer Datenpunkt gegenüber §43.7:** diese Maschine hat 18 registrierte Drucker (`Get-Printer`, u. a. "Microsoft Print to PDF") — die für Linux/CUPS gültige Erklärung "kein Zieldrucker vorhanden" trifft auf Windows nicht zu, was die bisherige Einordnung als reine Automatisierungsgrenze schwächt. Kein Root-Cause-Versuch in dieser Session (außerhalb des Validierungs-Scopes). |
...
1. **§43.7 `printer-dialog-probe`** bleibt offen, jetzt mit dem zusätzlichen Datenpunkt "18 Drucker registriert, Dialog erscheint trotzdem nicht" — sollte in einer künftigen Windows-Session gezielt root-caused werden (z. B. `Process Explorer`/`Spy++` gegen den tatsächlichen Fensterbaum während des Aufrufs, oder Prüfen, ob `QPageSetupDialog::exec()` unter Qt 6.11/MSVC auf einen COM-Print-Provider wartet, der in dieser RDP-Session nicht verfügbar ist).
...
Cluster-1-Fixes (is-shown?/enable-Kaskade), Geometrie-Block (§32) und Scroll-Block (§33/§34) waren laut `CLAUDE.md`-Checkpoint-Tabelle bereits 2026-09-17 auf Windows validiert. Mit dieser Session sind zusätzlich die vier 2026-09-17/18/19 nachgezogenen Commits (macOS-Menüband-Collapse, `QApplication`-Teardown, macOS-Menü-Platzhalter, Stdout-Noise) sowie §35 (Toolbar-Overlap) erstmals auf Windows geprüft. **Der gebündelte Cross-Platform-Durchlauf (Windows/macOS/Linux) ist damit für alle bis 2026-09-19 bekannten Befunde komplett**, mit den zwei oben genannten offenen Ausnahmen (§43.7, Zombie-Root-Cause).
