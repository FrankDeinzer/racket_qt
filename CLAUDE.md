...
**Nächste geplante Sitzung (Nutzerentscheidung 2026-09-14): `docs/HACKING.md` §35** —
oben links im Editorfenster werden zwei Toolbar-Controls übereinander gezeichnet
(`Untitled`/Dateiname und `Undock` an derselben Stelle), auf Linux in jedem Screenshot
stabil reproduzierbar, Belegbild `docs/2026-09-14-3_toolbar-overlap-linux.png`.
Möglicherweise derselbe Befund wie das gestreifte Rechteck (§35 Hypothese 1, unbelegt).
**Erste Pflichtmessung: Nativ-Gate** (DrRacket ohne `PLT_QT`) — ob der Befund überhaupt
Qt-spezifisch ist, ist ungemessen. Details je
