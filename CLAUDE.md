...
> **⚠ Offener Shim-Rebuild für Windows und macOS (Stand 2026-09-16).**
> Drei Fixes haben je neue Exporte eingeführt: `shim_window_set_resize_cb`
> (`resizeEvent`-Fix, §32), `shim_canvas_set_wheel_cb` (Scroll-Block, §33) und
> `shim_clipboard_set_text`/`shim_clipboard_get_text`/`shim_clipboard_has_text`
> (Zwischenablage, §36). Auf **Windows und macOS** muss `qt-shim` nach dem nächsten Pull
> **einmalig neu gebaut** werden — **ein** Rebuild deckt alle drei ab (Linux ist gebaut).
...
`shim_clipboard_{set,get,has}_text`; ein Rebuild deckt alle drei ab).
...
(Edit-Menü → Copy) blieb bei der Verifikation unbewiesen — Copy/Cut im Edit-Menü waren
trotz aktiver Selektion durchgehend ausgegraut, deckt sich mit §34.7s offenem Befund zu
nicht nachgeführten Menü-Enable-States; die zugrundeliegende `clipboard-driver%`-API
ist im selben Prozess direkt bestätigt. Scope bewusst nur Text — `get-bitmap-data`/
