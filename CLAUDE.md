...
| htdp-Lackmustest (`2htdp/image`, big-bang, `test-engine`) | 🟡 Windows, 2026-07-14 (§23): `2htdp/image` + big-bang einwandfrei (Kern-Wette „Racket treibt, Pump blockiert nie" bestätigt). `test-engine`-Dock-Crash reproduziert 4/4 unter Qt, 0/3 nativ — **kein htdp-lib-Bug, sondern lokalisierte `wx/qt`-Lücke** (`is-shown?`/Resize-Timing während Tab-Erstellung), Root-Cause bis `on-tab-change`-Dispatch eingegrenzt, Fix offen für künftige Session. macOS/Linux-Validierung separater Prompt |
